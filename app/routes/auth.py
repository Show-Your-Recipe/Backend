...
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        ...
        new_user = Users.create(session, auto_commit=True, pw=hash_pw, email=reg_info.email, name=reg_info.name, nick_name=reg_info.nick_name, phone_number=reg_info.phone_number)
...
async def login(sns_type: SnsType, user_info: UserLogin):
    if sns_type == SnsType.email:
